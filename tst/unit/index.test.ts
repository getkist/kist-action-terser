...
import plugin, { Action, ActionPlugin, JavaScriptMinifyAction } from "../../src/index.js";
...
describe("package entry point", () => {
    it("re-exports every action it provides", () => {
        expect(JavaScriptMinifyAction).toBeDefined();
        expect(new JavaScriptMinifyAction()).toBeInstanceOf(Action);
    });
    ...
    it("registers its actions by the name pipeline steps use", () => {
        ...
        expect(Object.keys(actions).sort()).toEqual(["JavaScriptMinifyAction"]);
        expect(new actions.JavaScriptMinifyAction()).toBeInstanceOf(JavaScriptMinifyAction);
    });
});
